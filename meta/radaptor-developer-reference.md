...
| **first-party package** | A sajat framework, CMS es theme csomagok (4 db). Sajat GitHub repo-juk van, a fejlesztes az app-localis `packages-dev/...` alatt tortenik. | framework, cms, portal-admin, so-admin |
| **packages-dev** | App-localis, gitignore-olt editable first-party package checkout-ok gyujtohelye. Minden csomagnak sajat nested Git repo-ja van itt. | `packages-dev/core/framework/` |
...
| **local override mod** | Maintainer-only runtime allapot: a gitignored `radaptor.local.json` atiranyitja a first-party csomag forrasat egy `packages-dev/...` alatti editable checkout-ra. Csak akkor ervenyes, ha a package-dev compose override is aktiv. | `radaptor.local.json` + `radaptor.local.lock.json` |
...
| **nested repo** | Git repo egy masik Git repo-n belul (nem submodule). A `packages-dev/` alatti csomagok ilyenek. Csak a dedikalt kivetelek megengedettek (lasd AGENTS.md). | `packages-dev/core/cms/.git` |
...
modositod, hanem az app-localis editable repo-t:

- `packages-dev/core/framework/`
- `packages-dev/core/cms/`
- `packages-dev/themes/portal-admin/`
- `packages-dev/themes/so-admin/`
...
package-dev compose override szukseges:
...
"Az editable package tartalom" **HELYETT**: "a `packages-dev/core/framework/` konyvtar jelenlegi tartalma".
...
"dev"` + logikai `location` (pl. `core/framework`), es a package-dev compose
override-t kell hasznalni.

| Csomag | Committed mod | Registry path | Editable path (local override eseten) |
|---|---|---|---|
| core/framework | registry | `packages/registry/core/framework/` | `packages-dev/core/framework/` |
| core/cms | registry | `packages/registry/core/cms/` | `packages-dev/core/cms/` |
| themes/portal-admin | registry | `packages/registry/themes/portal-admin/` | `packages-dev/themes/portal-admin/` |
| themes/so-admin | registry | `packages/registry/themes/so-admin/` | `packages-dev/themes/so-admin/` |
