...
{
	/** @var array<string, array{cache_key: string, lock: array{lockfile_version: int, packages: array<string, array<string, mixed>>, path: string, base_dir: string}} > */
	...
	{
		$cache_key = self::buildCacheKey($path);

		if ($cache_key !== null && isset(self::$_cache[$path]) && self::$_cache[$path]['cache_key'] === $cache_key) {
			return self::$_cache[$path]['lock'];
		}
	...
	private static function buildCacheKey(string $path): ?string
	{
