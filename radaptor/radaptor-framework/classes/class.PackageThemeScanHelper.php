...
class PackageThemeScanHelper
{
	...
	private static function getActiveRoots(): array
	{
		...
		foreach ($packages as $package) {
			...
			$type = PackageTypeHelper::normalizeType($package['type'], 'Theme package');
			$id = PackageTypeHelper::normalizeId($package['id'] ?? null, 'Theme package');
	...
	{
		$search_patterns = [
			rtrim($package_root, '/') . '/theme/ThemeData.*.php',
			rtrim($package_root, '/') . '/ThemeData.*.php',
		];
